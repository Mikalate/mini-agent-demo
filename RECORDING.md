...
6. 使用明确标注的低 token 预算触发滚动摘要；
7. read_docs 读取白名单文档与 search 精排；
8. 自进化经验（错误引导、经验库展示与跨 session 召回说明）；
9. Trace 摘要和默认离线测试。
...
若手动录制，建议只捕获标题为 `MiniAgentDemo` 的 Windows Terminal 窗口。录屏文件放入 `artifacts/`；演示用 `mini-agent-terminal-demo.mp4` 会随仓库提交，`*.mkv` 备用格式体积更大仍被 `.gitignore` 忽略。
...
本机最终采用终端回放方案，已生成 `artifacts/mini-agent-terminal-demo.mp4`：H.264、1600×900、约 127 秒、约 12 MB，覆盖五个工具、多 session、恢复、Context 压缩、read_docs、自进化经验、Trace 与 62 项离线测试；该 mp4 随仓库提交（`.gitignore` 仅忽略 mkv 备用格式）。已抽帧确认中文、工具链、session 恢复、滚动摘要、Trace 和 `62 passed, 5 deselected` 均可见。
